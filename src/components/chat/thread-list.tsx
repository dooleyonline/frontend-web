...
const getSenderName = (
  ...
  return (
    thread.participants.find((participant) => participant.id === senderId)
      ?.displayName ?? "Unknown"
  );
};

const getAvatarUrl = (thread: ChatThread, currentUserId: string) => {
  if (thread.isGroup) {
    return thread.participants.find((p) => p.id !== currentUserId)?.avatarUrl;
  }
  return thread.participants.find((p) => p.id !== currentUserId)?.avatarUrl;
};
...
export const ThreadList = (props: ThreadListProps) => {
  ...
  const filteredThreads = useMemo(() => {
    ...
    return threads.filter((thread) => {
      const title = normalize(buildThreadTitle(thread, currentUserId));
        ...
        <div className="flex flex-col">
          ...
          {!isLoading && !isError && filteredThreads.length === 0 ? (
            ...
            filteredThreads.map((thread) => {
              ...
              const title = buildThreadTitle(thread, currentUserId);
              const avatarUrl = getAvatarUrl(thread, currentUserId);
              const preview = lastMessage
                ? `${getSenderName(thread, lastMessage.senderId, currentUserId)}: ${
                    lastMessage.body
                  }`
                : "No messages yet";
